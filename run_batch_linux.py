...
WRAP = textwrap.dedent("""
    import sys, pathlib
    # Ensure the project directory is on sys.path so imports resolve
    _PROJECT_DIR = str(pathlib.Path({target!r}).parent.resolve())
    if _PROJECT_DIR not in sys.path:
        sys.path.insert(0, _PROJECT_DIR)

    import json, runpy, atexit, importlib, matplotlib
